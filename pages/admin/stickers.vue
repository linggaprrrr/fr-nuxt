...
// ── Create (bulk for image, single for emoji) ──────────────────────────────
function onBulkFileChange(e: Event) {
  const files = Array.from((e.target as HTMLInputElement).files ?? [])
  bulkFiles.value = files
  // Auto-set label to first filename if label empty
...
<template>
  <div>
    ...
    <div class="d-flex align-center justify-space-between mb-4 flex-wrap gap-2">
      <div>
        <h5 class="text-h5 font-weight-bold">Stickers</h5>
        <p class="text-caption text-medium-emphasis mt-1">
          Kelola sticker emoji & image yang muncul di kiosk editor foto.
        </p>
      </div>
    ...
    <VCard flat border>
      <VCardText>
        ...
        <div v-if="!loading && stickers.length === 0" class="text-center pa-12 text-medium-emphasis">
          <VIcon size="56" class="mb-3" color="grey-lighten-1">bx bxs-smile</VIcon>
    ...
    <VDialog v-model="showCreate" max-width="560" persistent>
      <VCard>
        <VCardTitle class="d-flex align-center gap-2 pa-4 pb-2">
          <VIcon color="primary">bx bxs-smile</VIcon>
          Tambah Sticker
          <VSpacer />
        ...
        <VCardText class="pa-4">
          ...
          <div class="d-flex gap-3 mb-4">
            <div
              class="type-card"
              :class="{ 'type-card-active': createType === 'image' }"
              @click="createType = 'image'"
            >
              <VIcon size="28" :color="createType === 'image' ? 'primary' : 'grey'">bx bx-image</VIcon>
            ...
            <div
              class="type-card"
              :class="{ 'type-card-active': createType === 'emoji' }"
              @click="createType = 'emoji'"
            >
          ...
          <template v-if="createType === 'image'">
            ...
            <div
              ...
              @dragover.prevent
              @drop.prevent="e => { bulkFiles = Array.from(e.dataTransfer?.files ?? []); }"
            >
              <VIcon size="36" color="primary" class="mb-2">bx bx-cloud-upload</VIcon>
            ...
            <div v-if="bulkFiles.length > 0" class="mt-3">
              ...
              <div class="bulk-preview-grid">
                <div
                  v-for="(url, i) in createPreviewUrls"
                  :key="i"
                ...
                >
                  ...
                  <VBtn icon variant="text" size="x-small" class="bulk-preview-del" @click="removeBulkFile(i)">
                    <VIcon size="12" color="error">bx bx-x</VIcon>
                  </VBtn>
                </div>
              </div>
            </div>

            <!-- Label (untuk bulk = dipakai semua, atau per-file menggunakan filename) -->
            <VTextField
              v-model="createLabel"
        ...
        <VCardActions class="pa-4">
          <VSpacer />
          <VBtn variant="text" @click="showCreate = false; resetCreateForm()" :disabled="isSubmitting">Batal</VBtn>
          <VBtn
            color="primary"
            variant="elevated"
    ...
    <VDialog v-model="showEdit" max-width="420">
      <VCard>
        ...
        <VCardActions class="pa-4">
          <VSpacer />
          <VBtn variant="text" @click="showEdit = false" :disabled="isSubmitting">Batal</VBtn>
